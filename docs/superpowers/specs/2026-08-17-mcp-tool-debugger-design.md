...
所有短暂、非阻塞的全局反馈，以及需要用户即时确认的轻量操作，统一使用应用根节点的 Toast。页面不得创建私有 Toast，也不得使用浏览器原生 `alert`、`confirm` 或 `prompt`。

- 全局只挂载一个 Toast viewport，固定在视口正中央，同时最多显示 4 条；其定位不受页面滚动和业务布局影响。
- 成功、错误、警告和加载状态使用同一中性表面、语义图标和左侧状态线，不使用整块高饱和底色。
- 关闭按钮固定在右上角，并提供“关闭通知”的可访问名称。
- 普通反馈默认在 2.5 秒后关闭；加载状态和需要用户决策的 Toast 持续显示，直到操作完成、取消或关闭。
- 需要确认时，说明文本位于上方，操作按钮位于底部右侧；次要的“取消”在左，主要操作在右。按钮文案必须描述实际动作，例如“允许试运行”或“允许执行”。
- Toast 不参与页面文档流，出现或消失时不得改变编辑器、卡片或表单布局。
- 表单字段错误、长期状态和必须保留的诊断继续使用就地反馈；Toast 只承载短暂反馈与轻量确认。
- 试运行和正式执行检测到 `tools.call` 时均使用同一警告 Toast。用户确认后必须重新读取并校验最新脚本和参数，不得执行确认前缓存的旧快照。
