...
export function RunHistory({ api, projectId, tabId, onOpen }: Props) {
  ...
  useEffect(() => {
    ...
    void api.listRuns(projectId).then((page) => { if (generation.current !== current) return;
  ...
  async function more(): Promise<void> {
    ...
    try { const page = await api.listRuns(projectId, requested); if (generation.current !== current) return;
  ...
  const visible = runs.filter((run) => tabId === undefined || run.tabId === tabId);
