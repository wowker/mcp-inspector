...
describe("run routes", () => {
  ...
  it("validates extended filters and exposes an idempotent project-fenced pin mutation", async () => {
    const list = vi.fn(() => ({ runs: [summary], nextCursor: null }));
    const response = await createRunRoutes(fake({ list })).request(
      `/${projectId}/runs?runId=${runId}&status=failed&origin=REPLAY&source=AUTHORING_STANDALONE&pinned=true&createdFrom=2026-09-01T00%3A00%3A00.000Z&limit=25`);
    expect(response.status).toBe(200);
    expect(list).toHaveBeenCalledWith(projectId, undefined, {
      runId, status: "failed", origin: "REPLAY", source: "AUTHORING_STANDALONE", pinned: true,
    ...
    expect((await createRunRoutes(fake()).request(`/${projectId}/runs?runId=bad`)).status).toBe(400);
